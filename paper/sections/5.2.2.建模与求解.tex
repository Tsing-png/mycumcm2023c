...
对每个品类的日销售量序列$S_{i,t}$，采用Prophet模型\cite{Taylor2018}分解：
\begin{equation}
...
\end{equation}
其中$P = (1+\alpha)C_i$。第一项为正价销售收入，第二项为未售出商品按折扣价$kP$清仓的收入，第三项为含损耗的进货总成本。$k=0.66$为从打折数据估算的折扣价与正价比值的中位数。该模型属于具有加性需求不确定性的经典报童框架\cite{Dana2001,Qin2011}。
...
优化问题为$\max_{\alpha \geq 0,\; 0 \leq Q \leq Q_{\max}} \pi_i(Q, \alpha)$，其中$Q_{\max} = 1.5 \times \max_\tau S_{i,\tau}$为订货量上限。求解采用二维网格搜索（加成率35步$\times$订货量因子35步），每点5000次Monte Carlo抽样估计期望利润。网格边界和步长经两轮试验确定：首轮在更宽范围内以10步粗搜定位最优解所在区域，次轮在该区域周围的$[\alpha^* \pm 15\%, Q^* \pm 20\%]$范围内加密至35步精搜。该策略在保证精度的同时控制了总运行时间（六个品类完整搜索约4分钟）。

\subsubsection{模型求解}

基于上述模型，本节利用Python（Prophet, scipy）进行求解。Prophet在2023年5月31日前数据训练，预测31天至7月1日，2023年6月（30天）作为留出集评估。Prophet的傅里叶级数阶数经初步筛选：周周期6阶、年周期10阶，变点先验尺度0.05——这是默认值，试了0.01和0.25，前者趋势太硬、后者过拟合噪声，0.05在两者间平衡。节假日效应包含全部中国法定假日及调休窗口。
...
设置ARIMA+固定加成baseline：以ARIMA替代Prophet\cite{NieYX2024}，加成率固定为历史均值，订货量$Q = \hat{D}^{\text{ARIMA}} / (1-L)$。相同利润函数和参数下，Baseline单日总利润为723元。M1增益（+304\%）来自两方面：Prophet更准确的季节性预测（如ARIMA将水生根茎类预测为$13.4$ kg而Prophet预测$30.6$ kg），以及加成率优化。
